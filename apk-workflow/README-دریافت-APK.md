...
بیلد خودکار الان خراب است، چون فایل workflow در `.github/workflows/build-apk.yml` دستور `checkout` روی شاخه‌ای به نام `arena/01a0367d-chart-dna` دارد که حذف شده. (من نمی‌توانم آن فایل را درست کنم: گیت‌هاب اجازهٔ تغییر پوشهٔ `.github/workflows` را به ربات نمی‌دهد؛ فقط صاحب ریپو می‌تواند.)
